...
# static generation
shutil.copyfile('../static/agency.txt', './agency.txt')
...
#         continue
#     schdate = datetime.datetime.fromtimestamp(schedule['timetable']['startdate']).timetuple().tm_yday
#     if schdate != today_datea:
#         continue
#     n_schedule.append(schedule)
# ds_compl['schedule'] = n_schedule

# print(len(ds_compl['schedule']))
...
for schedule in ds_compl['schedule']:
    ...
    # all_schedules_ids.append(schedule['id'])
    # if schedule['route']['next_schedule_id'] in all_schedules_ids:
    #     all_schedules_ids.remove(schedule['route']['next_schedule_id'])
...
# json.dump(scroutes, open('trap.json', 'w'))
...
for trip in trips_rows:
    ...
    #     if not nextSid:
    #         break
    #     if nextSid not in indexed_schedules:
    #         break
    #     if nextSid in used_stops:
    #         break

    #     used_stops.append(nextSid)

    #     sch = indexed_schedules[nextSid]
    ...
    #     nextSid = sch['route']['next_schedule_id']
    #     counter += 1



fileWriter(calendar_head, [calendar_row], 'calendar.txt')
fileWriter(stops_head, stops_rows, 'stops.txt')
fileWriter(routes_head, routes_rows, 'routes.txt')
fileWriter(stop_times_head, stop_times_rows, 'stop_times.txt')
fileWriter(trips_head, trips_rows, 'trips.txt')


shutil.make_archive(f'../{folder}', 'zip', './')